export default [
  {
    src: 'https://element.eleme.cn/favicon.ico',
    text: 'element',
    url: 'https://element.eleme.cn/#/zh-CN'
  },
  {
    src: 'https://element-plus.gitee.io/images/element-plus-logo-small.svg',
    text: 'element-plus',
    url: 'https://element-plus.gitee.io/zh-CN/'
  },
  {
    src: 'https://qn.antdv.com/favicon.ico',
    text: 'antd vue',
    url: 'https://www.antdv.com/docs/vue/introduce-cn/'
  },
  {
    src: 'https://gw.alipayobjects.com/zos/rmsportal/rlpTLlbMzTNYuZGGCVYM.png',
    text: 'antd react',
    url: 'https://ant.design/docs/react/introduce-cn'
  },
  {
    src: 'https://gw.alipayobjects.com/zos/bmw-prod/69a27fcc-ce52-4f27-83f1-c44541e9b65d.svg',
    text: 'antd-mobile',
    url: 'https://mobile.ant.design/zh'
  },
  {
    src: 'https://img.alicdn.com/tfs/TB1Jr2ASQvoK1RjSZFDXXXY3pXa-200-200.png',
    text: 'ng-zorro',
    url: 'http://ng.ant.design/docs/introduce/zh'
  },
  {
    src: 'https://gw.alipayobjects.com/zos/rmsportal/vCgyalwcqlpHLsJOIMwA.ico',
    text: 'ng-zorro-mobile',
    url: 'https://ng.mobile.ant.design/#/docs/introduce/zh'
  },
  {
    src: 'https://img.yzcdn.cn/vant/logo.png',
    text: 'vant',
    url: 'https://vant-contrib.gitee.io/vant/v3/#/zh-CN'
  },
  {
    src: 'https://www.uviewui.com/favicon.ico',
    text: 'uview',
    url: 'https://www.uviewui.com/'
  },
  {
    src: 'https://www.naiveui.com/assets/naivelogo.93278402.svg',
    text: 'Naive UI',
    url: 'https://www.naiveui.com/zh-CN/light'
  },
  {
    src: 'https://varlet.gitee.io/varlet-ui/varlet_icon.png',
    text: 'varlet-ui',
    url: 'https://varlet.gitee.io/varlet-ui/#/zh-CN/home'
  },
  {
    src: 'https://unpkg.byted-static.com/latest/byted/arco-config/assets/favicon.ico',
    text: 'arco-design',
    url: 'https://arco.design/react/docs/start'
  },
  {
    src: 'https://mui.com/static/favicon.ico',
    text: 'MUI',
    url: 'https://mui.com/zh/getting-started/installation/'
  },
  {
    src: 'https://cdc.cdn-go.cn/tdc/latest/images/tdesign.svg',
    text: 'TDesign',
    url: 'https://tdesign.tencent.com/'
  },
  {
    src: 'https://img12.360buyimg.com/imagetools/jfs/t1/101920/21/28459/834912/62870b04E0e1f20ec/17425bea5318a22c.png',
    text: 'NutUI-JDT',
    url: 'https://nutui.jd.com/jdt/#/',
    title: '京东科技移动端组件库'
  },
  {
    src: 'https://vuetifyjs.com/favicon.ico',
    text: 'Vuetify',
    url: 'https://vuetifyjs.com/zh-Hans/',
    title: 'Material Design 框架'
  },
  {
    src: 'https://lf9-static.semi.design/obj/semi-tos/images/favicon.ico',
    text: 'Semi Design',
    url: 'https://semi.design/zh-CN/',
    title: '由抖音前端与 UED 团队维护，易于定制的现代化设计系统，帮助设计师与开发者打造高质量产品。'
  },
  {
    src: 'https://file.iviewui.com/file/favicon.ico',
    text: 'iView',
    url: 'http://iview.talkingdata.com/#/components/guide/install'
  }
]
